...
// Set the objective for a minimization problem: min 0.5*xT*H*x+fT*x
    ...
    if (hessian.rows()!=_variableVect.size())
    {
    ...
    try
    {
        ...
        GRBQuadExpr obj = 0;
        for (auto row=0; row<_variableVect.size(); row++) {
...
// Add a linear inequality constraint: Ax <= b
    ...
    if ((A.cols()!=_variableVect.size()) || (A.rows()!=b.rows()) || (b.cols()!=1))
    {
...
bool GUROBIsolver::modifyConstraint(const Eigen::MatrixXd& A, const Eigen::VectorXd& b, std::vector<GRBConstr>& constraint)
{
    ...
    if ((A.cols()!=_variableVect.size()) || (A.rows()!=b.rows()) || (b.cols()!=1) || (A.rows()!=constraint.size()))
    {
    ...
    try
    {
        ...
        for (auto constr=0; constr<constraint.size(); constr++) {
...
bool GUROBIsolver::printConstraint(const std::vector<GRBConstr>& constraint) {
    ...
    try
    {
        for (auto constr=0; constr<constraint.size(); constr++) {
            ...
            std::cout << "\t LHS: [";
            for (auto var=0; var<_variableVect.size()-1; var++) {
