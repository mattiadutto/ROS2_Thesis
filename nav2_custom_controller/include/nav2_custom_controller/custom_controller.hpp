...
class CustomController : public nav2_core::Controller
{
    ...
    public:
        ...
        void calcLineEquation(const geometry_msgs::msg::Point32 &p1,  
            const geometry_msgs::msg::Point32 &p2,
            std::vector<std::vector<float>> &A_matrix,
            std::vector<std::vector<float>> &b_vect);

        void inflate_constraint(const std::vector<float> &A_matrix_row,
            const float &b_vect,
            const geometry_msgs::msg::Point32 &centroid);

        bool isViolated(const geometry_msgs::msg::Point32 &point,
            const std::vector<float> &A_matrix_row,
            const float &b_vect);

        void compute_violated_constraints(const geometry_msgs::msg::Point32 &p_centroid,
            const std::vector<std::vector<float>> &A_matrix,
            const std::vector<std::vector<float>> &b_vect);

        void compute_most_violated_constraints();

        float compute_distance_between_lines(const std::vector<float> &A_matrix_row,
            const float &b_vect,
            const float &b_vect_inflated);

        float compute_distance_to_violated_constraint(const float &x_coord, 
            const float &y_coord, 
            const std::vector<float> &A_matrix_row,
            const float &b_vect);
